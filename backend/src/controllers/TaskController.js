...
export const updateTaskStatus = async (req, res) => {
    ...
    try {
        ...
        if (task.status === status) {
            return res.status(200).json({ message: 'No change in task status', success: false, task });
        }
        ...
        return res.status(200).json({ message: 'Task status successfully updated', success: true, task });
    } catch (err) {
        return res.status(500).json({ message: err.message });
    }
};
