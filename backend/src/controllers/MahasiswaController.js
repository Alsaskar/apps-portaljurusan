...
export const add = async (req, res) => {
    ...
    if(fullname === ''){
        return res.status(500).json({message: 'Nama Lengkap tidak boleh kosong', success: false})
    }else if(nim === ''){
        return res.status(500).json({message: 'NIM tidak boleh kosong', success: false})
    }else if(jenisKelamin === ''){
        return res.status(500).json({message: 'Jenis Kelamin tidak boleh kosong', success: false})
    }else if(kotaLahir === ''){
        return res.status(500).json({message: 'Kota Lahir tidak boleh kosong', success: false})
    }else if(tglLahir === ''){
        return res.status(500).json({message: 'Tanggal Lahir tidak boleh kosong', success: false})
    }else if(agama === ''){
        return res.status(500).json({message: 'Agama tidak boleh kosong', success: false})
    }else if(kota === ''){
        return res.status(500).json({message: 'Kota tidak boleh kosong', success: false})
    }else if(angkatan === ''){
        return res.status(500).json({message: 'Angkatan tidak boleh kosong', success: false})
                ...
                if(cekNoHp > 0){
                    ...
                    if(cekNim > 0){
                        ...
                        if(cekEmail > 0){
                            ...
                            return res.status(200).json({message: 'Berhasil menambahkan data', success: true})
                        }
                    }
    ...
    if(fullname === ''){
        ...
        if(validateEmail(email)){ // jika email valid
            if(validateNumberPhone(noHp)){ // jika nomor telefon valid
                ...
                return res.status(200).json({message: 'Data berhasil di edit', success: true})
            }else{
                return res.status(500).json({message: 'No HP tidak valid', success: false})
            }
        }else{
...
export const updatePassword = async (req, res) => {
    ...
    try{
        ...
        return res.status(200).json({ message: 'Password berhasil diubah. Password sesuai tanggal lahir Mahasiswa ', success: true })
    }catch(err){
        return res.status(500).json({ message: err.message })
    }
}
...
export const addDetail = async (req, res) => {
    ...
    try{
        const cekDetailMahasiswa = await DetailMahasiswa.count({ where: { mahasiswaId: idMahasiswa } })

        if(cekDetailMahasiswa > 0){ // jika data sudah ada, maka update data
            ...
            return res.status(200).json({ message: 'Detail berhasil di ubah', success: true })
        }else{ // jika data belum ada, maka tambah data
            ...
            return res.status(200).json({ message: 'Detail berhasil ditambahkan', success: true })
        }
    }catch(err){
        return res.status(500).json({ message: err.message })
    }
